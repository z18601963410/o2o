...
function listHeadTitleManagementInfo() {
    ...
    $.ajax({
        url: "/o2o/superadmin/listheadlines",
        type: "post",
        dataType: 'json',
        data: {
...
function headTitleManagementEdit() {
    ...
    headLine.lineId = encodeURIComponent($("#headTitleManagementEdit_lineId")
        .val());
    //headLine.lineName = encodeURIComponent($("#headTitleManagementEdit_lineName").val());
    headLine.lineName = $("#headTitleManagementEdit_lineName").val();
    headLine.lineLink = encodeURIComponent($(
    ...
    //头条图片
    var lineImg = $("#headTitleManagementEdit_lineImg").val();
    //头条对象
    //var formData = new FormData();
    //formData.append("headTitleManagementEdit_lineImg", headTitleManagementEdit_lineImg);
    //formData.append("headLineStr", JSON.stringify(headLine));
    var options = {
            type: 'post',
            processData: false,//processData默认为true，为true时，提交不会序列化data。
            data: {
                headLineStr: JSON.stringify(headLine),
                thumbnail: lineImg
            },
            url: '/o2o/superadmin/modifyheadline',// 请求的action路径
            error:
...
function batch(flag) {
    if ($('#headTitleManagementTable').datagrid('getSelected')) {
        ...
        $.messager.confirm('删除提示', '你确定永久删除下列消息吗?<br/>' + lineNames.join(','),
            function (r) {
                if (r) {
                    var url = 'removeheadlinelist?headLineIdListStr='
                        + encodeURIComponent(JSON.stringify(jsonList));
                    changeStatus(url);
                }
            });
    }
